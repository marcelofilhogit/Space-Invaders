...
(define (criarNave)
  (define x (/ largura 2.))
  (define y (- altura (* 3. tamanhoNave)))
  (novaNave x y))

;Cria novos Monstros e suas posição inicial
(define (criarMonstro)
  (for/list ([i 36])
    (define x (+ 30 (* 50 (remainder i 9)))) ; 8 colunas
    (define y (+ 30 (* 30 (remainder i 4)))) ; 3 linhas
    (novoMonstro x y)))
...
(define (atualizarNave w)
  ...
  (define moverNave
    (cond [d                   p]
          [(tecla? 'left)  (cond [(> x 6) (nave (- x 2.) y tamanho morto?)][else (nave x y tamanho morto?)])]
          [(tecla? 'right) (cond [(< x (- largura (+ tamanhoNave 16))) (nave (+ x 2.) y tamanho morto?)][else (nave x y tamanho morto?)])]
...
(define (atualizarMonstro i)
  ...
  (monstro novo-x y tamanho nova-patrulha-x nova-rapidez-x))
...
(define (dispararTiroMonstro w)
  ...
  (define (monstroDisparouTiro i)
    ...
    (if (and (> (random) 0.995)
        ...
        (tiro x (+ y tamanho 1) 3 (- (random) 0.5) 2.)
        #f))
...
;Desenhar os corpos no mundo w para o contexto de desenho dc
...
(define (desenharMundo w dc)
  ...
  ;(list (send dc draw-bitmap (make-object bitmap% "nave1.png") 50 10))
  )
...
(define (desenharCorpos bs dc)
  (for ([b bs])
    (match-define (corpo x y s) b)
    ;(define c (if (nave? b) (if (nave-morto? b) "red" "green") "black"))
    ;(read-bitmap "nave1.png")
    ;(send dc set-brush (new brush% [color c] [style 'solid]))
    ;(send dc draw-bitmap (make-object bitmap% "nave1.png") x y s s)
    (define c (if (nave? b) (if (nave-morto? b) "red" "green") "black"))
    
    (if (equal? (if (nave? b) (if (nave-morto? b) "red" "green") "black") "red")
       (send dc draw-bitmap (make-object bitmap% "game-over.jpg") 10 110)
        (cond
          [(= s 3) (send dc draw-bitmap (make-object bitmap% "tiro.png") x y)]
          [(= s 14) (send dc draw-bitmap (make-object bitmap% "nave2.png") x y)]
          [(= s 15) (send dc draw-bitmap (make-object bitmap% "nave1.png") x y)]
          [else (send dc draw-bitmap (make-object bitmap% "nave1.png") x y)]
          ))))
...
;(new message% [parent frame] [label (read-bitmap "nave1.png")])
